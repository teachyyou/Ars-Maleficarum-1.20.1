...
public class RiteOfKramerTorchCreation extends CircleRitual {
    ...
    public void executeRitual(BlockState pState, Level pLevel, BlockPos pPos, Player pPlayer, RitualCoreEntity riteCore) {
        ...
        if (allComponentsConsumed && ticks%20==0) {
            ...
            ((ServerLevel)pLevel).sendParticles(ParticleTypes.FLAME, d0, d1, d2, 100, 0,0.1D,0,0.2);
        }
        if (allComponentsConsumed && ticks/20==5) {
            ...
            ((ServerLevel)pLevel).sendParticles(ParticleTypes.ENCHANT, d0, d1, d2, 100, 0,0.5D,0,0.2);
            ((ServerLevel)pLevel).sendParticles(ParticleTypes.ENCHANT, d0, d1, d2, 100, 0,1D,0,0.2);
            pLevel.playSound(null, pPos, ModSounds.GLUTTONY_DEMON_DIED.get(), SoundSource.HOSTILE,1F,1F);
            possiblyDemon.get().discard();
        }


        if (allComponentsConsumed && ticks/20.0 == 6) {
            double d0 = staff.position().x;
            double d1 = staff.position().y;
            double d2 = staff.position().z;
            ...
            staff.setItem(new ItemStack(ModItems.SILVER_DAGGER.get())); //TODO Поместить сюда потом огненный посох
