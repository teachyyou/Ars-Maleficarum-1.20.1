...
public class CrimsonChalk extends Item {
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        if (!pContext.getLevel().isClientSide()) {
            ...
            if (pContext.getLevel().getBlockState(pContext.getClickedPos()).is(ModBlocks.CRIMSON_CHALK_SYMBOL.get())) {
                int variant = pContext.getLevel().getBlockState(pContext.getClickedPos()).getValue(ChalkSymbol.VARIANT);
                pContext.getLevel().setBlock(pContext.getClickedPos(), ModBlocks.CRIMSON_CHALK_SYMBOL.get().defaultBlockState().setValue(ChalkSymbol.VARIANT,(variant+1)%11).setValue(ChalkSymbol.FACING, glyphDirection),3);
            }
                ...
                pContext.getLevel().setBlock(pContext.getClickedPos().above(), ModBlocks.CRIMSON_CHALK_SYMBOL.get().defaultBlockState().setValue(ChalkSymbol.VARIANT,random.nextInt(11 )).setValue(ChalkSymbol.FACING, glyphDirection),3);
            }
            else {
